...
if True:
    ...
    import run
    from walytis_beta_api import Block, Blockchain, get_latest_blocks, decode_long_id
    import walytis_beta_api
    walytis_beta_api.log.PRINT_DEBUG = True

    from walytis_beta_api.blockchain_blocks import BlocksMetadata, BlockchainBlocks
...
def test_add_blocks() -> None:
    ...
    print(blockchain.get_num_blocks())


def test_blocksmetadata():
    blocks = BlocksMetadata.from_block_ids(
        get_latest_blocks(blockchain.blockchain_id, long_ids=True)
    )
    block_ids = blocks.get_blocks_ids()
    block_id = bytearray(block_ids[0])
    block_metadata = blocks.get_block_metadata(block_ids[0])

    print(mark(
        block_id in get_latest_blocks(blockchain.blockchain_id, long_ids=True),
    ),
        "get_blocks_ids")
    print(mark(
        block_id == block_metadata.long_id
        and block_metadata.creation_time == decode_long_id(block_id)["creation_time"]
    ), "BlockMetadata")
